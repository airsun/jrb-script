...
function evalValue(name) {
    ...
    var expr = name.replace(nameRe, function (matched,name) {
...
function readyTemplate() {
    ...
    re = /\$\{([\w+-\\*\/()]+)\}/gi;
    // expRe = /[+-/*]/i;
    var value = template.replace(re, function (matched, name) {
        //重复的name，跳过
        if (valueMap[name] == " ") {
            return
        }
        valueMap[name] = " ";
        // if (name.match(expRe) == null) {
            document.write("<div>" + name + ": <input type='text' name='" + name + "' title='" + name + "' onBlur='updateValue(this.name,this.value)'/></div>");
        // }
    });
}
